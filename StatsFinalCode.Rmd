...
sim_doubles_point <- function(team1, team2, data) {
  ...
  diff <- team1_avg_utr - team2_avg_utr
  prob1 <- 1 / (1 + exp(-2.6402 * diff))
  sim <- rbinom(1, 1, prob1)
...
sim_court_logit <- function(player1, player2, data) {
  ...
  prob_player1_wins <- 1 / (1 + exp(2.6402 * (player2_utr - player1_utr)))
  
  # Simulate the outcome probabilistically
  simv <- rbinom(1, 1, prob_player1_wins)
  if (simv == 1) {
    return(player1)
  } else {
    return(player2)
...
simulate_team_match <- function(team1, team2, data) {
  ...
  for (pos in 1:6) {
    ...
    court_winner <- sim_court_logit(player1, player2, data)
...
simulate_bracket_new <- function(teams, data, df) {
  ...
  team_pairs <-
    matrix(teams, nrow = length(teams) / 2, byrow = TRUE)
  
  # Simulate matches for each pair
  length <- nrow(team_pairs)
  if (length == 1) {
    rd <- "Champion"
  } else if (length == 2) {
    rd <- "Final"
  ...
  for (i in 1:nrow(team_pairs)) {
    ...
    winner <- simulate_team_match(team1, team2, data)
    df[df$team == winner, rd] <- df[df$team == winner, rd] + 1
  ...
  cat("\n")
  return(simulate_bracket_new(winners, data, df))
}

simulate_sandbag <- function(teams, data, team, df) {
  data[data$Team == team, "Lineup.Pos"] <-
    data[data$Team == team, "Lineup.Pos"] %% 6 + 1
  simulate_bracket_new(teams, data, df)
}

simulate_sandbag_unc <- function(teams, data, team, df) {
  data[data$Team == team, "Lineup.Pos"] <- c(2, 1, 3, 4, 5, 6)
  simulate_bracket_new(teams, data, df)
}
```
...
```{r}
# Usage example:
# all_teams <- unique(ordered_teams$Team)
# Get all the unique teams from the data


#simulate the tournament played "straight"
num_columns <- 6
teams <- unique(data$Team)
df <-
  data.frame(teams, matrix(0, nrow = length(teams), ncol = num_columns))
names(df) <-
  c("team", "RO32", "RO16", "QF", "SF", "Final", "Champion")
for (i in 1:1000) {
  df <- simulate_bracket_new(teams, data, df)
}

#simulate efficacy of sandbagging for a specific team
teams <- unique(data$Team)
df <-
  data.frame(teams, matrix(0, nrow = length(teams), ncol = num_columns))
names(df) <-
...
team <- "California"
for (i in 1:1000) {
  df <- simulate_sandbag(teams, data, team, df)
}

#determine whether North Carolina's lineup switch was effective
NC <- 0
NCS <- 0
data[data$Team == "North Carolina", "Lineup.Pos"] <-
  c(2, 1, 3, 4, 5, 6)
for (i in 1:10000) {
  winner <- simulate_team_match("North Carolina", "NC State", data)
  if (winner == "NC State") {
    NCS <- NCS + 1
  } else{
    NC <- NC + 1
